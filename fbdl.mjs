...
const BYPASS_API = "https://api.nexray.eu.cc/tools/bypass/cf";
...
async function getBypassCookie() {
  try {
    const response = await fetch(BYPASS_API, {
      method: "POST",
    ...
    if (data.status === "success" && data.cookies) {
      const cookieString = Object.entries(data.cookies)
        .map(([k, v]) => `${k}=${v}`)
        .join("; ");
      return cookieString;
    }
...
async function fbdl(url = DEFAULT_URL) {
  try {
    ...
    const cookieString = await getBypassCookie();
    ...
    if (html.includes("Just a moment")) {
      return {
        ...
        code: 403,
        creator: "rhmt",
        error: "Cloudflare challenge, API bypass mungkin butuh API key",
        raw_preview: html.slice(0, 500)
      };
    }

    return parseResult(html, input, response.status);
  } catch (err) {
    return {
      status: false,
      code: 500,
