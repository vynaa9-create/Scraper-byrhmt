 ...
 *
 * note :
 * baru pertama kali scrape cuy wkwkw :v jadi klo ada apa apa maklumi saja
 */

import process from "node:process";

const ENDPOINT_GENERATE = "https://api.picaapi.com/aigc/artguru/image/generate/v2";
const ENDPOINT_QUEUE = "https://api.picaapi.com/aigc/artguru/image/task-queue";

const HEADERS = {
  "content-type": "application/json",
  accept: "application/json, text/plain, */*",
  origin: "https://www.artguru.ai",
  referer: "https://www.artguru.ai/ai-text-to-image-generator/",
  "user-agent":
...
export default async function artguru(prompt, options = {}) {
  try {
    ...
    const first = await generateTask(cleanPrompt, options);
    ...
    const firstItem = Array.isArray(first.data?.data) ? first.data.data[0] : null;
    ...
    for (let i = 0; i < maxAttempts; i++) {
      ...
      const item = Array.isArray(polled.data?.data) ? polled.data.data[0] : null;
      if (!item) continue;
